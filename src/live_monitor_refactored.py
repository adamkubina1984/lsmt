...
def read_csv_any(path: Path) -> pd.DataFrame:
    if not path.exists():
        return pd.DataFrame()
    df = pd.read_csv(path)
    # sloupce na lowercase
    df.columns = [str(c).lower() for c in df.columns]
    # normalizace sloupce s časem
    ...
    def run(self):
        while not self._stop_evt.is_set():
            try:
                ...
                # 3) načtení dat a poslání výsledku do fronty
                ...
                conf = float(df["signal_strength"].iloc[-1])

                # Pošli data zpět do hlavního vlákna
                self.q.put({
                    "df": df,
                    "new_bar": True,
                    "last_dt": last_dt,
...
class LiveMonitor(tk.Tk):
    ...
    def update_from_payload(self, payload):
        ...
        # 5) Spodní panel: confidence + threshold + risk
        self.ax_conf.plot(x, conf_a, color='tab:blue', label='confidence')
